#!/usr/bin/env python3
"""
요약/결론 성격의 청크만 추출해 Qwen API로 최종 통합 브리핑을 생성합니다.
(Gemini 딥리서치용은 generate_gemini_briefing_deepresearch.py 참고)
...
예시:
    python3 scripts/generate_briefing.py \
      --input data/reports/2026-04-15/rag/chunks.jsonl \
      --output knowledge/daily/2026-04-15-qwen-briefing.md \
      --max-chunks 80
"""
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="요약 관련 청크만 추출해 Qwen API로 통합 브리핑을 생성합니다."
    )
    parser.add_argument(
        "--input",
        required=True,
        help="입력 JSONL 파일 경로 (예: data/reports/2026-04-15/rag/chunks.jsonl)",
    )
    parser.add_argument(
        "--output",
        required=True,
        help="응답을 저장할 출력 파일 경로 (예: knowledge/daily/2026-04-15-qwen-briefing.md)",
    )
    parser.add_argument(
        "--prompt",
        default=DEFAULT_PROMPT,
        help="LLM에 전달할 시스템성 요청 문구",
    )
    parser.add_argument(
        "--head-text-limit",
        type=int,
...
def select_briefing_rows(rows: List[Dict[str, Any]], head_text_limit: int, min_chunks: int, max_chunks: int) -> Tuple[List[Dict[str, Any]], List[Dict[str, Any]]]:
    ...
    # 3차: max_chunks 범위 내에서 score 높은 순으로 채움
    if len(selected) < max_chunks:
        for row in scored_rows:
            if len(selected) >= max_chunks:
                break
...
def call_qwen(api_key: str, model: str, prompt: str, merged_text: str, max_tokens: int) -> Tuple[str, str]:
    try:
...
def main() -> None:
    ...
    print(f"📦 총 chunks: {len(rows)} / 후보: {len(scored_rows)} / 선택: {len(selected_rows)}")
    merged_text = merge_chunks(selected_rows)
    print(f"📝 병합 텍스트 길이: {len(merged_text):,} chars")
    print(f"🤖 Qwen 호출 중... (model={args.model})")

    qwen_output, resolved_model = call_qwen(
        api_key=api_key,
        model=args.model,
        prompt=args.prompt,
    ...
    output_path.write_text(qwen_output + "\n", encoding="utf-8")
    ...
    print(f"✅ Qwen 브리핑 저장 완료: {output_path}")
    print(f"🧾 메타 저장 완료: {meta_path}")
    print(f"🧠 선택 청크: {len(selected_rows)} / 리포트 {report_coverage}개 커버")
