...
if [[ "$SKIP_COLLECT" == "1" ]]; then
  log "📡 [1/7] 수집 단계 건너뜀 (--skip-collect)"
  run_optional_step "📝 [2/7] PDF 전문 텍스트화... (기존 수집 데이터 재사용)" \
    node scripts/dump-report-texts.js --date "$DATE"
...
if [[ "$MANUAL_LLM" == "1" ]]; then
  log "🧬 [5/7] 수동 LLM 모드: API 압축은 생략하고 PDF 요약 큐/프롬프트만 준비합니다."

  if report_queue_reason="$(should_run_llm_stage report-queue)"; then
    if run_soft_step "📚 [5/7] 수동 LLM용 PDF 요약 큐 생성... $report_queue_reason" \
      node scripts/build-report-summary-queue.js --date "$DATE"; then
      run_soft_step "🧭 [5/7] 수동 LLM용 리포트 선별 프롬프트 생성..." \
    ...
    if [[ "$local_status" == "20" ]]; then
      log "📚 [5/7] PDF 요약 큐 생성 스킵... $report_queue_reason"
    else
  ...
  if compress_reason="$(should_run_llm_stage compress)"; then
    ...
    local_status=$?
    if [[ "$local_status" == "20" ]]; then
      log "🧬 [5/7] 리포트 압축 스킵... $compress_reason"
    else
  ...
  if synthesis_reason="$(should_run_llm_stage synthesis)"; then
    if run_soft_step "🧩 [6/7] 일일 시황 종합... $synthesis_reason" \
    ...
    local_status=$?
    if [[ "$local_status" == "20" ]]; then
      log "🧩 [6/7] 일일 시황 종합 스킵... $synthesis_reason"
    else
...
if [[ "$MANUAL_LLM" == "1" ]]; then
  if advisory_reason="$(should_run_llm_stage advisory)"; then
    ...
    local_status=$?
    if [[ "$local_status" == "20" ]]; then
      log "🧠 [7/7] 어드바이저 프롬프트 생성 스킵... $advisory_reason"
    else
  ...
  if advisory_reason="$(should_run_llm_stage advisory)"; then
    ...
    local_status=$?
    if [[ "$local_status" == "20" ]]; then
      log "🧠 [7/7] 어드바이저 생성 스킵... $advisory_reason"
    else
      log "⚠️ 어드바이저 실행 여부 판단 실패: $advisory_reason"
    fi
  fi
